...
import {
  ...
  CP_GRAY_700, CP_GRAY_900, CP_TEXT_MUTED, CP_SHADOW_SM, CP_SHADOW_MOD,
  CpButton, CpIco, CpInput, CpIconBtn,
} from './lp-ui';
import { LpCalendar, LpLineItem } from './lp-form-bits';
import { LpClientBanner } from './lp-client-views';
...
/* ── ORDER EDIT MODAL ── */
export function LpOrderEditModal({ order, onClose, onSave }) {
  ...
  return (
    <div style={{ background:'#fff', minHeight:'100%', fontFamily:CP_F }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'32px 32px 48px' }}>
        ...
        <div style={{ background:'#fff', borderRadius:12, border:`1px solid ${CP_GRAY_200}`, boxShadow:CP_SHADOW_SM, overflow:'hidden' }}>
    ...
    <div
      ...
      <div style={{
        ...
        <div style={{ display:'flex', gap:12, padding:'32px 24px 24px', marginTop:'auto' }}>
          <CpButton kind="secondary" onClick={()=>setEditing(null)} style={{ flex:1, height:44 }}>Отмена</CpButton>
          <CpButton kind="primary" onClick={onSave} style={{ flex:1, height:44 }}>Сохранить</CpButton>
        </div>
      </div>
    </div>
  );
