/* ── DESIGN TOKENS ── */
const BR='#c94030', BRH='#a8331f', ACC='#e8a838', NAT='#7a9e7e', ARCH='#8a7a90';
...
const PKG = {
  ...
  lotok: { id:'lotok', label:'Лоток',          note:'≈0.8–1 кг',  unit:'лот.', type:'counter', step:2 }, // ← multiple of 2
};

/* ── PRODUCTS (chilled by default; "frozen" is a checkbox per item) ── */
const PRODUCTS = [
...
/* Order of rows on the printed бланк (production form) */
const BLANK_ROWS = PRODUCTS.map(p => p.blank);
...
const setStatus = (id, st) => updateOrder(id, { status: st });

const getClient = () => { try { return JSON.parse(localStorage.getItem(LS_C)||'null'); } catch { return null; } };
const saveClient = c => c ? localStorage.setItem(LS_C, JSON.stringify(c)) : localStorage.removeItem(LS_C);

const migrateCp = c => {
  // Legacy: had addresses[]; collapse to single address
...
const saveCounterparties = list => localStorage.setItem(LS_CP, JSON.stringify(list.map(migrateCp)));

const getBanner = () => {
  try { return JSON.parse(localStorage.getItem(LS_BANNER)||'null') || DEFAULT_BANNER; }
  catch { return DEFAULT_BANNER; }
};
const saveBanner = b => localStorage.setItem(LS_BANNER, JSON.stringify(b));
...
const RU_WEEK_FULL = ['воскресенье','понедельник','вторник','среда','четверг','пятница','суббота'];
...
const parseISO = s => { const [y,m,d]=s.split('-').map(Number); return new Date(y, m-1, d, 12, 0, 0, 0); };
const sameDay = (a,b) => isoDate(a) === isoDate(b);
const isWeekday = d => { const w=d.getDay(); return w!==0 && w!==6; };
...
/** First possible shipment date based on the 15:00 cutoff rule. */
const firstShipmentFrom = (now=new Date()) => {
  const n = new Date(now);
  const past = n.getHours() > 15 || (n.getHours() === 15 && n.getMinutes() > 0);
  // anchor day (today if before cutoff, otherwise tomorrow)
...
Object.assign(window, {
  BR, BRH, ACC, NAT, ARCH, DRK, FG2, FG3, BG, ALT, CRD, BRD, FD, FB, FL,
  PKG, PRODUCTS, BLANK_ROWS, STATUSES,
  getOrders, saveOrders, addOrder, updateOrder, setStatus,
  getClient, saveClient, getCounterparties, saveCounterparties, getBanner, saveBanner,
  genId, isoDate, parseISO, sameDay, isWeekday, addWorkDays, firstShipmentFrom,
  fmtDate, fmtShort, fmtLong, RU_MONTHS, RU_MONTHS_SHORT, RU_WEEK, RU_WEEK_FULL,
  inp, lbl, LpLogo, LpBadge,
});
