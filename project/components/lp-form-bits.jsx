...
function LpCalendar({ value, minDate, onChange }) {
...
function LpDeadlineHint({ shipDate }) {
...
/* ── QUANTITY FIELD ── */
function LpQtyField({ pkgId, value, onChange }) {
  const p = PKG[pkgId];
  if (p.type === 'kg') {
    return (
      <input
  ...
  // clamp counter to multiple of step (for lotok)
...
/* ── LINE ITEM ROW ── */
function LpLineItem({ item, idx, total, onChange, onRemove }) {
...
Object.assign(window, { LpCalendar, LpDeadlineHint, LpQtyField, LpLineItem });
